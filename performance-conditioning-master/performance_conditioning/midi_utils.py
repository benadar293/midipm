...
def midi_to_frames(midi, instruments, conversion_map=None, HOP_LENGTH=HOP_LENGTH):
    ...
    print('midi to frames with conversion map', conversion_map)
...
def midi_to_frames_with_vel(midi, instruments, conversion_map=None, HOP_LENGTH=HOP_LENGTH):
    n_keys = MAX_MIDI - MIN_MIDI + 1
    midi_length = int((max(midi[:, 1]) + 1) * SAMPLE_RATE)
    n_steps = (midi_length - 1) // HOP_LENGTH + 1
    n_channels = len(instruments) + 1
    label = torch.zeros(n_steps, n_keys * n_channels, dtype=torch.uint8)
    velocity = torch.zeros(n_steps, n_keys * n_channels, dtype=torch.uint8)
    ...
    for onset, offset, note, vel, instrument in midi:
        ...
        label[onset_right:frame_right, n_keys * chan + f] = 2
        label[frame_right:offset_right, n_keys * chan + f] = 1
        velocity[left:onset_right, n_keys * chan + f] = vel

        inv_chan = len(instruments)
        label[left:onset_right, n_keys * inv_chan + f] = 3
        velocity[left:onset_right, n_keys * inv_chan + f] = vel
        label[onset_right:frame_right, n_keys * inv_chan + f] = 2
        label[frame_right:offset_right, n_keys * inv_chan + f] = 1
...
def frames2midi(save_path, onsets, frames, vels,
                ...
                inst_mapping=None, pitch_shift=None):
    print('frames 2 midi', onsets.shape, frames.shape, vels.shape)
    p_est, i_est, v_est, inst_est = extract_notes_np(onsets, frames, vels,
                                        onset_threshold, frame_threshold)
    if pitch_shift is not None:
        p_est += pitch_shift
        print('applied pitch shift frames2midi')
    p_ret, i_ret, v_ret, inst_ret = p_est, i_est, v_est, inst_est
    ...
    print('inst set', inst_set, 'inst map', inst_mapping)
    ...
    print('inst set mapped', inst_set)
    save_midi(save_path,
              p_est_lst, i_est_lst, v_est_lst,
              inst_set)
    return p_ret, i_ret, v_ret, inst_ret

'''
Pitch already in hz and instervals scaled to real time.
'''
def notes2midi(save_path, p_est, i_est, v_est, inst_est,
                inst_mapping=None):
    print('pset', p_est.shape, p_est.dtype)

    # inst_set = set(inst_est)
    # 2707 edited:
    inst_est = [el for el in np.squeeze(inst_est, axis=1)]
    inst_set = set(inst_est)
    # end 2707 edited
    ...
    v_est_lst = [v_est_lst[ins] for ins in inst_set if ins in v_est_lst]
    assert len(p_est_lst) == len(i_est_lst) == len(v_est_lst) == len(inst_set)
    inst_set = [inst_mapping[e] if inst_mapping is not None else e for e in inst_set]
    save_midi(save_path,
              p_est_lst, i_est_lst, v_est_lst,
              inst_set)
...
def parse_midi_multi(path, force_instrument=None, skip_drum=True):
        ...
        if hasattr(message, 'channel'):
            if message.channel == DRUM_CHANNEL:
                if skip_drum:
                    ...
                    print('not skipping drum', message)
...
def render_frames(labels, velocities, instruments,
                  shift=0, SPF=HOP_LENGTH, fl=None, sfid=None):
    reset_synth(fl)
    num_frames = len(labels)
    for c, instr in enumerate(instruments):
        fl.program_select(c, sfid, 0, instr)

    s = np.zeros(2 * num_frames * SPF, dtype=np.int16)
    for i, (F, V) in enumerate(zip(labels, velocities)):
        for C_ind, C in enumerate(instruments):
            frame, vel = F[C_ind * N_KEYS: (C_ind + 1) * N_KEYS], V[C_ind * N_KEYS: (C_ind + 1) * N_KEYS]
            onsets = (frame == 3).nonzero()[0]
            offsets = (frame == 1).nonzero()[0]
            for note in onsets:
                fl.noteon(C_ind, note + MIN_MIDI + shift, vel[note + shift])
            for note in offsets:
                fl.noteoff(C_ind, note + MIN_MIDI + shift)
        samps = fl.get_samples(SPF)
        s[i * 2 * SPF: (i + 1) * 2 * SPF] = samps
    return s


def save_midi_alignments_and_predictions(save_path, data_path, inst_mapping, onset_label,
                                         aligned_onsets, aligned_frames,
                                         onset_pred_np, frame_pred_np, prefix='', use_time=True, scaling=HOP_LENGTH / SAMPLE_RATE):
    ...
    frames2midi(save_path + '/' + data_path.replace('.flac', '').split('/')[-1] + prefix + '_alignment_' + time_now + '.mid',
                ...
                inst_mapping=inst_mapping, scaling=scaling)
    ...
    #                   64. * predicted_onsets[:, -N_KEYS:], scaling=scaling)


    # Pitch prediction, with choice of most likely instrument for each detected note.

    max_pred_onsets = max_inst(onset_pred_np) if len(inst_mapping) > 1 else predicted_onsets
    frames2midi(save_path + '/' + data_path.replace('.flac', '').split('/')[-1] + prefix + '_pred_inst_' + time_now + '.mid',
        ...
        # onset_max_label = np.maximum(max_pred_onsets[:, : inst_only], aligned_onsets[:, : inst_only])
        # frames_max_label = np.maximum(np.tile(predicted_frames[:, : inst_only], (1, len(inst_mapping))), aligned_frames[:, : inst_only])
        # vel_max_label = 64. * onset_max_label
        # print('saving max aligned and pred')
        # frames2midi(save_path + '/' + data_path.replace('.flac', '').split('/')[-1] + prefix + '_label_' + time_now + '.mid',
        #             onset_max_label, frames_max_label,
        #             vel_max_label,
        #             inst_mapping=inst_mapping, scaling=scaling)